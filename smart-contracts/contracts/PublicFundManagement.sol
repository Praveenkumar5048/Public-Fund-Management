...
contract PublicFundManagement {
    ...
    enum ProposalState { Created, UnderAuthorityVoting, PublicVoting, Approved, Rejected, InProgress, Completed }
    enum StageState { NotStarted, InProgress, Completed }
    ...
    event PublicVoted(uint256 proposalId, address voter, bool vote, string comment);
    event ProposalApproved(uint256 proposalId);
    event ProposalRejected(uint256 proposalId);
    event StageAmountReleased(uint256 proposalId, uint256 stageNumber, uint256 amount);
    event StageReportSubmitted(uint256 proposalId, uint256 stageNumber, string report);
    ...
    modifier onlyAuthority() {
        require(authorities[msg.sender], "Only authorities can call this function");
        _;
    }
     ...
     */
    function closePublicVoting(uint256 _proposalId) external onlyAdmin proposalExists(_proposalId) {
        Proposal storage proposal = proposals[_proposalId];
        require(proposal.state == ProposalState.PublicVoting, "Proposal is not in public voting state");
        ...
        if (proposal.publicYesVotes > proposal.publicNoVotes) {
            ...
            require(contractBalance >= proposal.totalAmount, "Contract does not have enough funds for this proposal");
     ...
     * @param _proposalId ID of the proposal
     */
    function releaseStageAmount(uint256 _proposalId) external onlyAdmin proposalExists(_proposalId) {
        Proposal storage proposal = proposals[_proposalId];
        ...
        if (currentStage == 0) {
            ...
            require(proposal.stages[currentStage-1].state == StageState.Completed, "Previous stage not completed");
        }
        
        require(currentStage < proposal.totalStages, "All stages already released");
        ...
        require(contractBalance >= stage.amount, "Contract does not have enough funds for this stage");
    ...
    function createProposal(
        ...
        require(_stageAmounts.length > 0 && _stageAmounts.length <= 3, "Must have 1-3 stages");
        ...
        require(totalStageAmount == _totalAmount, "Sum of stage amounts must equal total amount");
     ...
     */
    function authorityVoteOnProposal(uint256 _proposalId, bool _vote) external onlyAuthority proposalExists(_proposalId) {
        Proposal storage proposal = proposals[_proposalId];
        require(proposal.state == ProposalState.UnderAuthorityVoting, "Proposal is not under authority voting");
        require(!proposal.hasAuthorityVoted[msg.sender], "Authority has already voted");
     ...
     */
    function submitStageReport(uint256 _proposalId, uint256 _stageNumber, string calldata _report) external proposalExists(_proposalId) {
        Proposal storage proposal = proposals[_proposalId];
        require(msg.sender == proposal.recipient, "Only recipient can submit report");
        require(_stageNumber < proposal.totalStages, "Invalid stage number");
        
        Stage storage stage = proposal.stages[_stageNumber];
        require(stage.state == StageState.InProgress, "Stage is not in progress");
     ...
     */
    function voteOnStage(uint256 _proposalId, uint256 _stageNumber, bool _approve) external onlyAuthority proposalExists(_proposalId) {
        Proposal storage proposal = proposals[_proposalId];
        require(proposal.state == ProposalState.InProgress, "Proposal is not in progress");
        require(_stageNumber < proposal.totalStages, "Invalid stage number");
        
        Stage storage stage = proposal.stages[_stageNumber];
        require(stage.state == StageState.InProgress, "Stage is not in progress");
        require(bytes(stage.report).length > 0, "Report not submitted yet");
        require(!stage.hasVoted[msg.sender], "Authority has already voted on this stage");
    ...
    /**
     * @dev Vote on a proposal as a member of the public
     * @param _proposalId ID of the proposal
     * @param _vote True for yes, false for no
     * @param _comment Comment on the vote
     */
    function publicVoteOnProposal(uint256 _proposalId, bool _vote, string calldata _comment) external proposalExists(_proposalId) {
        Proposal storage proposal = proposals[_proposalId];
        require(proposal.state == ProposalState.PublicVoting, "Proposal is not open for public voting");
     ...
     */
    function getProposalInfo(uint256 _proposalId) external view proposalExists(_proposalId) returns (
     ...
     */
    function getStageInfo(uint256 _proposalId, uint256 _stageNumber) external view proposalExists(_proposalId) returns (
        uint256 amount,
        string memory report,
        uint256 voteCount,
        StageState state
    ) {
        require(_stageNumber < proposals[_proposalId].totalStages, "Invalid stage number");
